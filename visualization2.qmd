...
```{r}
my_country <- map_data("world", region = "Cuba")
my_country |> glimpse()
my_country |> count(group, subregion)
p <- 
  ggplot() +
...
p1 <- p + coord_quickmap()
p1
# add some data to map
p1 +
  geom_point(aes(x = -80, y = 21.75), colour = "red", size = 10)
# limit the area plotted
p1 +
  coord_quickmap(xlim = c(-85, -82.5), ylim = c(21.5, 23)) +
  geom_point(aes(x = -83.5, y = 22), colour = "red", size = 10)
```
